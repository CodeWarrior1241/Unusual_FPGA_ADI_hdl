...
#   cd {path/to/deps/hdl/projects/fmcomms2/au15p}
...
proc build_all {} {
    ...
    # Enable the AU15P bank-86 LVDS workaround inside the shared
    # axi_ad9361_lvds_if.v (PSEUDO_DIFF tx_clk + tx_data[0], SINGLE_ENDED
    # rx_data[5], plus 4 extra IDELAYCTRLs). AXAU15 does not define this
    # macro and gets the stock 6-pair LVDS generate loops.
    set_property verilog_define {AU15P_BANK86_WORKAROUND=1} [current_fileset]
    set_property verilog_define {AU15P_BANK86_WORKAROUND=1} [current_fileset -simset]
    puts "INFO: Set verilog_define AU15P_BANK86_WORKAROUND=1 on synth + sim filesets"
    ...
    create_bd_cell -type ip -vlnv [resolve_ip_vlnv xilinx.com:ip:clk_wiz 6.0] $ecs_clock_300_mhz
    set_property -dict [list \
    ...
    create_bd_cell -type ip -vlnv [resolve_ip_vlnv xilinx.com:ip:xlconcat 2.1] rx_data_p_concat
    ...
    create_bd_cell -type ip -vlnv [resolve_ip_vlnv xilinx.com:ip:xlconcat 2.1] rx_data_n_concat
    ...
    create_bd_cell -type ip -vlnv [resolve_ip_vlnv xilinx.com:ip:xlslice 1.0] tx_data_p_hp_slice
    ...
    create_bd_cell -type ip -vlnv [resolve_ip_vlnv xilinx.com:ip:xlslice 1.0] tx_data_n_hp_slice
    ...
    create_bd_cell -type ip -vlnv [resolve_ip_vlnv xilinx.com:ip:xlslice 1.0] tx_data_p_hd_slice
    ...
    create_bd_cell -type ip -vlnv [resolve_ip_vlnv xilinx.com:ip:xlslice 1.0] tx_data_n_hd_slice
    ...
    # ─── Propagate AU15P_BANK86_WORKAROUND to OOC synth runs ──────────────
    # The verilog_define set on [current_fileset] above only reaches the
    # top-level user-RTL synthesis. axi_ad9361 (and any BD-instantiated IP
    # that references the macro-gated lvds_if.v) is synthesized OOC inside
    # its own per-IP synth_run, with its own fileset that does NOT inherit
    # the parent project's verilog_defines. Inject the macro into every IP
    # synth run that pulls in axi_ad9361 sources. Without this, the AU15P
    # bank-86 ifdef blocks in axi_ad9361_lvds_if.v stay excluded and the
    # PSEUDO_DIFF / SINGLE_ENDED pin handling vanishes from the netlist.
    ...
    foreach run $ip_synth_runs {
        ...
        set new "$existing -verilog_define AU15P_BANK86_WORKAROUND=1"
        set_property STEPS.SYNTH_DESIGN.ARGS.MORE_OPTIONS $new $run
        puts "INFO: Injected -verilog_define AU15P_BANK86_WORKAROUND=1 into $run"
    }
