// ***************************************************************************
// Asynchronous AXI4-Stream FIFO for the Ti375C529 FMCOMMS2 design: the CDC
...
// block RAM (Ti375 RAM10 blocks) instead of fabric registers —
// depth 512 costs the same block count down to depth 512, so nothing shallower
...
// TKEEP(4) + TSTRB(4) + TLAST(1) = 41 bits, which maps to 3 RAM10 blocks
// per FIFO at 512 deep (x20/x20/x1 lane split). TKEEP and TSTRB
...
// interface compatibility with the axau15 design. To reclaim block RAM
...
// efx_reset_gen (RST_PULSE_CYCLES — the same pulse-prolong role
...
module axis_async_fifo #(
  ...
  // -------------------------------------------------------------------------
  // FIFO controller + block RAM storage
  // -------------------------------------------------------------------------
...
// registered read (RamReadLatency=1). syn_ramstyle="block_ram" pins storage
// to Efinix RAM10 blocks (never registers) — the role olo_base_ram_sdp played.
