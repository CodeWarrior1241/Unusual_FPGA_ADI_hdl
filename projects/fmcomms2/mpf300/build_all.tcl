...
#     location) or from the Libero GUI via Project -> Execute Script.
...
# MegaVault — the online repository is not reachable from this machine.
# Verified on Libero 2025.2 and 2026.1 (default: 2026.1).
# Override knobs for other machines / Libero releases (e.g. running
# 2025.2 instead of the default 2026.1):
#   LIBERO_MEGAVAULT          path to the MegaVault registered with Libero
#   PF_CCC_VERSION            catalog version of PF_CCC to generate
#   PF_INIT_MONITOR_VERSION   catalog version of PF_INIT_MONITOR
# Unset, the core versions are auto-discovered as the newest available in
# the MegaVault; the pinned defaults below (from the 2025.2 vault) are the
# last resort.
#
###############################################################################
...
# Offline IP vault. LIBERO_MEGAVAULT overrides the dev-box default; the
# vault is only consulted here for core-version discovery and a sanity
# check -- core generation itself uses the vault registered with Libero
# (Project -> Vault/Repositories settings).
if {[info exists ::env(LIBERO_MEGAVAULT)]} {
    variable megavault $::env(LIBERO_MEGAVAULT)
} else {
    variable megavault "/media/fpgadev/Dev_Tools/Microchip_FPGA/2026.1/MegaVault/Libero_SoC_v2026.1_MegaVault"
}

# Catalog-core version resolution: explicit env var wins; otherwise the
# newest version present in the MegaVault (Components/Actel/SgCore/<core>/
# <version>/); otherwise the pinned default. create_and_configure_core
# fails hard on a version the vault does not ship, so on a different
# Libero/MegaVault release (e.g. 2026.1) the discovery -- or the env
# override -- replaces the 2025.2 pins.
proc resolve_core_version {core env_var default} {
    global megavault
    if {[info exists ::env($env_var)]} {
        puts "INFO: $core version $::env($env_var) (from \$$env_var)"
        return $::env($env_var)
    }
    set versions {}
    foreach d [glob -nocomplain -type d \
                   [file join $megavault Components Actel SgCore $core *]] {
        set v [file tail $d]
        if {[regexp {^\d+(\.\d+)*$} $v]} { lappend versions $v }
    }
    if {[llength $versions] > 0} {
        set best [lindex [lsort -command {package vcompare} $versions] end]
        if {$best ne $default} {
            puts "INFO: $core version $best (newest in vault; pinned default $default)"
        }
        return $best
    }
    return $default
...
variable PF_CCC_version [resolve_core_version PF_CCC PF_CCC_VERSION "2.2.222"]
variable PF_INIT_MONITOR_version \
    [resolve_core_version PF_INIT_MONITOR PF_INIT_MONITOR_VERSION "2.0.308"]
...
proc build_all {} {
    ...
    if {![file exists $megavault]} {
        ...
        puts "WARNING: (set LIBERO_MEGAVAULT to your MegaVault path, and/or"
        puts "WARNING:  PF_CCC_VERSION / PF_INIT_MONITOR_VERSION to versions"
        puts "WARNING:  your registered vault actually ships)"
    }
