...
// ***************************************************************************
//
// Efinix port of axi_ad9361_cmos_if
//
// CMOS (single-ended) interface for AD9361/AD9364 transceiver.
//
// This module is functionally identical to the Xilinx version. The only
// difference is that it instantiates Efinix-specific I/O primitives.
//
// INTERFACE DESIGNER SETUP:
// -------------------------
// Configure the following in Efinix Interface Designer:
//
// RX (GPIO inputs with DDR):
//   - rx_clk_in: GPIO input, route to global clock network
//   - rx_frame_in: GPIO input with DDR capture
//   - rx_data_in[11:0]: GPIO inputs with DDR capture (12 pins)
//
// TX (GPIO outputs with DDR):
//   - tx_clk_out: GPIO output with DDR
//   - tx_frame_out: GPIO output with DDR
//   - tx_data_out[11:0]: GPIO outputs with DDR (12 pins)
//
// Control (single-ended outputs):
//   - enable: GPIO output
//   - txnrx: GPIO output
//
// ***************************************************************************
...
module axi_ad9361_cmos_if #(
  ...
  // drp interface signals - not functional on Efinix
  ...
  generate if (CLK_DESKEW) begin : gen_clk_deskew_r1

    reg adc_r1_mode_n_reg = 'd0;

    always @(negedge clk) begin
      adc_r1_mode_n_reg <= adc_r1_mode;
    end

    always @(posedge l_clk) begin
      rx_r1_mode <= adc_r1_mode_n_reg;
    end

  end else begin : gen_no_deskew_r1
  ...
  generate if (CLK_DESKEW) begin : gen_clk_deskew_adc

    reg         adc_valid_n_reg = 'd0;
    reg [47:0]  adc_data_n_reg = 'd0;
    reg         adc_status_n_reg = 'd0;

    always @(negedge l_clk) begin
      adc_valid_n_reg <= adc_valid_p;
      adc_data_n_reg <= adc_data_p;
      adc_status_n_reg <= adc_status_p;
    end

    always @(posedge clk) begin
      adc_valid_int <= adc_valid_n_reg;
      adc_data_int <= adc_data_n_reg;
      adc_status_int <= adc_status_n_reg;
    end
  ...
  end else begin : gen_no_deskew_adc
  ...
  generate if (CLK_DESKEW) begin : gen_clk_deskew_dac

    reg [ 1:0]  tx_clk_n_reg = 'd0;
    reg [ 1:0]  tx_frame_n_reg = 'd0;
    reg [11:0]  tx_data_0_n_reg = 'd0;
    reg [11:0]  tx_data_1_n_reg = 'd0;

    always @(negedge clk) begin
      tx_clk_n_reg <= tx_clk_p;
      tx_frame_n_reg <= tx_frame_p;
      tx_data_0_n_reg <= tx_data_0_p;
      tx_data_1_n_reg <= tx_data_1_p;
    end

    always @(posedge l_clk) begin
      tx_clk <= tx_clk_n_reg;
      tx_frame <= tx_frame_n_reg;
      tx_data_0 <= tx_data_0_n_reg;
      tx_data_1 <= tx_data_1_n_reg;
    end

  end else begin : gen_no_deskew_dac
  ...
  generate if (CLK_DESKEW) begin : gen_clk_deskew_ctrl

    reg enable_int_n_reg = 'd0;
    reg txnrx_int_n_reg = 'd0;

    always @(negedge clk) begin
      enable_int_n_reg <= enable_int;
      txnrx_int_n_reg <= txnrx_int;
    end

    always @(posedge l_clk) begin
      enable_int_p <= enable_int_n_reg;
      txnrx_int_p <= txnrx_int_n_reg;
    end

  end else begin : gen_no_deskew_ctrl
  ...
  generate if (USE_SSI_CLK == 1) begin : gen_ssi_clk
  ad_data_clk #(
    .SINGLE_ENDED (1)
  ) i_clk (
    .rst (1'd0),
