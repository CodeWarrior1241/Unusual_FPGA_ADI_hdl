...
// ***************************************************************************
//
// Microchip PolarFire port of the Xilinx axi_ad9361_lvds_if. The framing,
// delineation, tx serialization and ensm logic are identical to the Xilinx
// version; the device-specific I/O (IBUFDS/IDDR/ODDR/IDELAY, IDELAYCTRL,
// BUFG) is replaced with PolarFire equivalents (INBUF_DIFF/DDR_IN/DDR_OUT/
// OUTBUF_DIFF/CLKBUF_DIFF) via the polarfire/common ad_data_* modules.
//
// The dynamic IDELAY calibration interface has no PolarFire fabric
// equivalent here: delay readback returns zero and delay_locked is tied
// high (same approach as the Intel port). Interface timing closure relies
// on static SDC constraints instead of runtime tap calibration.
...
module axi_ad9361_lvds_if #(
  ...
  // with the Xilinx version; they have no function on PolarFire.
  ...
  // delay interface signals (no dynamic delay control on PolarFire)
  ...
  // receive data interface, inbuf -> ddr_in
  ...
  // receive frame interface, inbuf -> ddr_in
  ...
  // transmit data interface, ddr_out -> outbuf

  generate
  for (i = 0; i < 6; i = i + 1) begin: g_tx_data
  ad_data_out i_tx_data (
    .tx_clk (l_clk),
  ...
  // transmit frame interface, ddr_out -> outbuf

  ad_data_out i_tx_frame (
    .tx_clk (l_clk),
    .tx_data_p (tx_frame),
    .tx_data_n (tx_frame),
