...
// - Loads 1024 QPSK samples (compile.do auto-converts COE to hex)
...
//
// Same test sequence and pass criteria as ../xilinx/axi_ad9361_tb.v; the UUT
// is built from the PolarFire device interface (../../polarfire/) instead of
// the Xilinx one, so DDR I/O is fabric-emulated and the IDELAY controls are
// inert (delay_locked tied high, readback zero).
//
// Target: Microchip QuestaSim Pro / ModelSim Pro (ships with Libero SoC)
//==============================================================================
...
module axi_ad9361_tb;
    ...
    // Load Sample Data
    //==========================================================================
    // The compile.do script converts the COE file (shared with the Xilinx
    // sim) to a hex file before simulation starts.
    ...
    axi_ad9361 #(
        ...
        .FPGA_TECHNOLOGY(0),              // Unused by the PolarFire device interface
        .FPGA_FAMILY(0),
        .SPEED_GRADE(0),
        .DEV_PACKAGE(0),
    ...
    // Data flows through the UUT's own TX path (fabric-emulated DDR output
    // registers + OUTBUF_DIFF). The RX path captures it with INBUF_DIFF and
    // fabric-emulated DDR capture registers.
    //
